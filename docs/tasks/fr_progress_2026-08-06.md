...
| FR-ID | 機能 | 状況 | 詳細 |
|---|---|---|---|
| FR-VEH-001 | 車両基本情報表示 | 🟡 | `app/(public)/vehicles/[slug]/page.tsx`で一部項目（走行距離・エンジン・ミッション・外装色・内装色・VIN）のみ表示。年式・排気量・馬力・トルク・車検・オーナー数・屋内保管・事故歴・シート素材など未表示 |
...
| FR-ID | 機能 | 状況 | 詳細 |
|---|---|---|---|
| FR-SRCH-001 | 絞り込み検索 | 🟡 | `lib/inventory/search.ts`のコメントに「主要項目のみ」と明記。実装済み：メーカー・車種・価格帯・年式帯・走行距離上限・ミッション・屋内保管のみ。未実装：シリーズ・世代・グレード・車検・排気量・馬力・トルク・オーナー数・内装色・外装色・シート素材・タグ（駆動＝drivetrainはDBにはあるが検索条件化されていない）。なお「燃料」はDBスキーマ自体に列が存在しない |
...
| FR-LINE-002 | 相談カテゴリ表示 | ❌ | 単一の「LINEで相談する」ボタンのみ。購入／修理／売却／部品／Jaguar全般／カーライフ相談のカテゴリ別導線は存在しない |
