      ...
      {Object.entries(grouped).map(([type, items]) => (
        <div key={type}>
          ...
          <div className="mt-1 flex max-h-40 flex-col gap-1 overflow-y-auto rounded-lg border border-neutral-200 bg-white p-2">
            {items.length === 0 && (
              <p className="text-foreground-muted px-1 py-2 text-base">
                候補がありません
              </p>
            )}
            {items.map((c) => (
              <label
                key={`${c.type}:${c.id}`}
                className="text-charcoal-900 flex min-h-11 items-center gap-2 rounded-md px-1 text-base hover:bg-neutral-100"
              >
                <input
                  type="checkbox"
                  checked={selectedKeys.has(`${c.type}:${c.id}`)}
                  onChange={() => toggle(c)}
                  className="text-primary-600 focus-visible:ring-primary-500/50 h-4 w-4 rounded border-neutral-300 focus-visible:ring-2"
                />
                {c.label}
              </label>
            ))}
